...
class Transformer(nn.Module):
    def __init__(self,input_dims=3,conv_out=64,num_heads=8,dim_ff=1024,encoder_layers=3,dropout=0.1):
    ...
    def forward(self,x):
        ...
        #print(x.shape)
        x=x.reshape(x.shape[0],self.conv_out*self.lout1)
        x=self.fc1(x)
        x=self.layernorm1(x)
        #x=self.batchnorm1(x)
        x=self.relu(x)
