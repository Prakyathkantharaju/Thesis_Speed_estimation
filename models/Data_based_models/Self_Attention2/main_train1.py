...
X=norm.transform(X)
X=X.reshape(-1,2,200)
dataset=IMU(X,Y,size=200,num_features=2)
...
for fold,(train_idx,val_idx) in enumerate(splits.split(np.arange(len(dataset)))):
        ...
        model=CNN(input_features=2,num_classes=1)
        model.to(device)
        optimizer=optim.Adam(model.parameters(),lr=3.5e-5)
        ...
        for epoch in range(num_epochs):
            ...
            if epoch>15 and test_loss<=0.0035:
