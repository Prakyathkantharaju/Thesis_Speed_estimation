...
from model2 import CNN,ResidualBlock
...
X=norm.transform(X)
X=X.reshape(-1,1,200)
dataset=IMU(X,Y,size=200,num_features=1)
...
for fold,(train_idx,val_idx) in enumerate(splits.split(np.arange(len(dataset)))):
        ...
        model=CNN(ResidualBlock,[1,3,6,3],num_classes=1)
        model.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            if epoch>20 and test_loss<=0.002:
