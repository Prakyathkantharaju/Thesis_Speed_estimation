...
def preprocess(window_int=1):
    ...
    for i in map.keys():
        ...
        x,y=split(data,map[i],window=window_int,sample_rate=200,vert_acc_i=0)
    ...
    for i in range(1,14):
        ...
        x,y=split(data['data'],data['label'],window=window_int,sample_rate=200,vert_acc_i=0)
        X=np.concatenate((X,x),axis=0)
        Y=np.concatenate((Y,y),axis=0)
    ...
    norm.fit(X)
    X=norm.transform(X)
    X=X.reshape(X.shape[0],3,int(200*window_int))
